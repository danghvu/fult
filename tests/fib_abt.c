...
void expand_fibonacci(void *arguments)
{
    ...
    if (n <= 2) {
        ...
        if (parent != NULL) {
            ...
            ABT_task_create(g_pool[rank], aggregate_fibonacci, agg_args, NULL);
        } else {
            args->result = 1;
        ...
        ABT_task_create(g_pool[rank], expand_fibonacci, exp_args1, NULL);
        ...
        ABT_task_create(g_pool[rank], expand_fibonacci, exp_args2, NULL);
    }
}
...
void aggregate_fibonacci(void *arguments)
{
    ...
    if (parent != NULL) {
        ...
        if (flag) {
            ...
            ABT_task_create(g_pool[0], aggregate_fibonacci, agg_args, NULL);
...
int fibonacci(int n)
{
    ...
    args->parent = NULL;
    ABT_task_create(g_pool[0], expand_fibonacci, args, NULL);
...
int main(int argc, char *argv[])
{
    ...
    n = argc > 1 ? atoi(argv[2]) : N;
    num_xstreams = argc > 2 ? atoi(argv[1]) : NUM_XSTREAMS;
    printf("# of ESs: %d\n", num_xstreams);
    ...
    for (i = 1; i < num_xstreams; i++) {
        ABT_xstream_create_basic(ABT_SCHED_RANDWS, num_xstreams, g_pool,

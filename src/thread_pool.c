...
/*
 * COPYRIGHT -- TODO: credit argobot source code properly.
 */
...
#define ABTU_free free
#define ABTU_malloc malloc
#define LOG_DEBUG(...) {}
#define ABTI_ASSERT assert
...
void ABTI_mem_init(ABTI_global *p_global)
{
    ...
    gp_ABTI_global->mem_sp_size = (8*1024*1024);
    gp_ABTI_global->mem_page_size = (2*1024*1024);
    gp_ABTI_global->huge_page_size = (2*1024*1024);
    gp_ABTI_global->mem_max_stacks = 65536;
    gp_ABTI_global->mem_lp_alloc = ABTI_mem_check_lp_alloc(ABTI_MEM_LP_MMAP_HP_RP);
    gp_ABTI_global->lock = 0;
    ...
    switch (lp_alloc) {
        ...
        case ABTI_MEM_LP_MMAP_HP_THP:
            ...
            if (p_page != MAP_FAILED) {
                ...
                posix_memalign((void**) &p_page, alignment, pg_size);
                if (p_page) {
                    ABTU_free(p_page);
        ...
        case ABTI_MEM_LP_MMAP_HP_THP:
            ...
            if ((void *)p_page != MAP_FAILED) {
                ...
                *p_is_mmapped = ABT_FALSE;
                ...
                posix_memalign((void**) &p_page, alignment, pgsize);
...
char *ABTI_mem_alloc_sp(ABTI_local *p_local, size_t stacksize)
{
    ...
    /* First stack */
    unsigned int first_pos = p_sph->id % num_stacks;
